        ...
        <div class="col-md-8">
            <div class="card">
				...
				<div class="card-body" style="overflow-y: scroll; overflow-x: scroll; height: 450px;">
					...
					<a href="/client/tambah" class="btn btn-primary">Input Client</a>
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            @foreach($client as $p)
                            <tr>
                                <td><img src='uploads/client/{{ $p->image }}' width="50px"></td>
                                <td>{{ $p->keterangan }}</td>
                                <td>{{date('d F Y', strtotime($p->created_at))}}</td>
                                <td>{{date('d F Y', strtotime($p->updated_at))}}</td>
                                <td>
                                    <a href="/client/edit/{{ $p->id }}" class="btn btn-warning">Edit</a>&emsp;
                                    <a href="/client/hapus/{{ $p->id }}" class="btn btn-danger">Hapus</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
					...
					<div class="box-footer clearfix">
						<div class="col-sm-12 col-md-5">
							Showing {{ $client->perPage() }} of {{ $client->total() }} entries
						</div>
